"""
Taurus Dashboard – Correspondance code SIC (SEC) → secteur GICS.

L'écran Modigliani-Miller applique un taux de coûts de détresse propre au
secteur (`capital_structure._SECTOR_DISTRESS_RATE`) : une entreprise
technologique détruit bien plus de valeur en faillite qu'un service public
réglementé.  SEC EDGAR publie un code SIC pour chaque déclarant ; ce module le
traduit dans la nomenclature sectorielle attendue par le moteur.
